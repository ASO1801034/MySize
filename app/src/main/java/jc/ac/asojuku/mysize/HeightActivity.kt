...
class HeightActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        spinner.onItemSelectedListener= //スピナーにアイテムを選んだ時の動きを持ったクラスの匿名インスタンスを代入
            object : AdapterView.OnItemSelectedListener{ //アイテムを選んだ時の動きを持ったクラスの継承クラスを定義して匿名インスタンスにする
                override fun onItemSelected( //アイテムを選んだ時の処理
                    parent: AdapterView<*>?, //選択が発生したビュー（スピナー）のこと
                ...
                override fun onNothingSelected(p0: AdapterView<*>?) { //アイテムを何も選択しなかった時の処理
                    //何もしない
                }
            }
        //シークバーの処理を定義する
        //共有プリファレンスから身長設定値を取得する(シークバー表示値のため)
        ...
        //ラジオボタンの処理を実装する
        //ラジオグループに選択された時に反応するコールバックメソッドを待機するリスナー設定
        radioGroup.setOnCheckedChangeListener{
            //2つの引数(第1引数:ラジオボタングル―プ)、第2引数:選択されたラジオボタンのidを受け取って実行する処理
                group,checkedId ->
